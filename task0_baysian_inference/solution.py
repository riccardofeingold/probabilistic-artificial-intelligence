...
def log_posterior_probs(x):
    ...
    log_P_H_i_prior = np.log(PRIOR_PROBS)
    log_likelihood = np.zeros(3)
    log_p = np.zeros(3)
    for i in range(3):
        log_likelihood[i] = np.sum(np.log(HYPOTHESIS_SPACE[i].pdf(x)))
    
    result1 = log_likelihood + log_P_H_i_prior
    result2 = np.exp(result1)
    result3 = np.sum(result2)
    
    log_P_x_prior = logsumexp(log_likelihood + log_P_H_i_prior)
    log_p = log_likelihood + log_P_H_i_prior - log_P_x_prior 
